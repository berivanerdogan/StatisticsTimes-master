...
namespace StatisticsTimes.Core.Map
{
    public class CoreMap<T> : EntityTypeConfiguration<T> where T : CoreEntity
    {
        public CoreMap()
        {
            ...
            Property(x => x.CreatedIP).HasColumnName("CreatedID").IsOptional();


            Property(x => x.ModifiedBy).HasColumnName("ModifiedBy").IsOptional();
            Property(x => x.ModifiedIP).HasColumnName("ModifiedID").IsOptional();
